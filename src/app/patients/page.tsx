  ...
  const fetchPatients = async () => {
    try {
      ...
      if (user?.role === UserRole.DOCTOR) {
        ...
        data = await apiClient.getPatients(user.hospitalId, user.id)
      } else if (user?.role === UserRole.HOSPITAL_ADMIN && user.hospitalId) {
        // For hospital admins, show patients from their hospital
        data = await apiClient.getPatients(user.hospitalId)
      } else {
        // For super admin, show all patients
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {patients.map((patient) => (
            <Card key={patient._id} className="card-hover">
              <CardHeader>
