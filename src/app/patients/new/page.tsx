...
import { UserRole, type Doctor, type Hospital } from "@/components/lib/types"
...
export default function NewPatientPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate hospital and doctor selection
      if (user?.role !== UserRole.DOCTOR) {
      ...
      const patientData = {
        ...
        ...(user?.role === UserRole.DOCTOR 
          ? { doctorId: user.id, hospitalId: user.hospitalId }
          : {
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to enroll patient"
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => {
      ...
      // If hospital is changed, reset doctor selection
      if (name === "hospitalId") {
        newData.doctorId = ""
      }
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {user?.role !== UserRole.DOCTOR && (
                  <div>
                    ...
                    >
                      <option value="">{formData.hospitalId ? "Select a doctor" : "Select a hospital first"}</option>
                      {doctors
                        .filter((d) => d.hospitalId === formData.hospitalId)
                        .map((doctor) => (
                          <option key={doctor._id} value={doctor._id}>
                            {doctor.name} - {doctor.specialization} ({doctor.designation})
                          </option>
                        ))}
                    </select>
                  </div>
